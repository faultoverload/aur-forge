#!/usr/bin/env bash
# aur-forge entrypoint — dispatches init|build|serve|help based on argv[1].
# All persistent state lives in /repo (served), /cache (chroots + ccache),
# /keys (GPG keyring), /pkglist (one package per line).
set -euo pipefail
