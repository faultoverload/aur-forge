...
#
# This script is also sourced by build.sh to expose GITHUB_API as a
# library constant. To avoid the source-time arg-check firing with $#=0
# (which would abort the calling script under set -e), the executable
# body lives inside main() and is only invoked when this script is run
# directly. Same pattern as srcinfo-diff.sh in this repo.
set -euo pipefail
...
# Without this guard, build.sh's `source scripts/open-quarantine-issue.sh`
# line would trigger the $#-lt-3 check under set -e and abort before
# reading the pkglist.
